...
class Image:
    ...
    def save(self, fp, format=None, **params):
        ...
        # ===== [omz] Optimization for Pythonista (iOS):
        if string.lower(os.path.splitext(filename)[1]) == '.png':
            import console
            from copy import copy
            image_to_save = self
            if image_to_save.mode != 'RGBA':
                image_to_save = image_to_save.convert('RGBA')
            image_data = copy(image_to_save.tostring())
...
def open(fp, mode="r"):
    ...
    if isStringType(fp):
        import __builtin__
        filename = fp
        #Addition for Pythonista to support loading built-in images more easily:
        if not os.path.isfile(filename):
            try:
                import scene
                built_in_image_path = scene.get_image_path(filename)
                if built_in_image_path is not None:
                    fp = built_in_image_path
                    filename = built_in_image_path
            except ImportError:
                pass
