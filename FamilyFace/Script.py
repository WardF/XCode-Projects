# Famly Face - Based on the 'Cascade' Pythonista demo.
#
...
class Game (Scene):
	def setup(self):
		...
		ipad = self.is_iPad() # self.size.w > 700
	...
	def random_tile(self,i):
		ipad = self.is_iPad()
		cols = 12 if ipad else 8
		rows = 12 if ipad else 10
		images = ['_sammyface', '_mommyface', '_daddyfacetmp','__lukeface2']
	...
	def new_game(self):
		...
		for i in xrange(cols * rows):
			tile = self.random_tile(i)
			#tile = Tile(images[randint(0, len(images)-1)],
			#            i % cols, i / cols)
			self.grid.append(tile)
		
		# Make a few random bombs.
		rand_index = []
		for i in xrange(3,6):
			rand_index.append(randint(0,len(self.grid)))
	
		for i in range(0,len(rand_index)):
			self.grid[rand_index[i]].image = '_thebomb'
		# End placing bombs
	...
	def neighbors(self, tile):
		...
		for direction in directions:
			...
			elif tile.image == '_thebomb' and neighbor is not None:
				result.append(neighbor)
		return result
	...
	def touch_began(self, touch):
		...
		for tile in self.grid:
			...
			if tile.hit_test(touch):
				if tile.image == '_thebomb':
					is_bomb = True
					self.select_bomb_from(tile, set())	
				else:
	...
	def new_top_row(self):
		new_tiles = False
		ipad = self.is_iPad()
		cols = 12 if ipad else 8
		rows = 12 if ipad else 10
		for col in xrange(cols):
			tile = self.tile_at(col,(rows-1))
			if tile is None:
				new_tiles = True
				tile = self.random_tile((rows-1) * cols+col)
				self.grid[(rows-1) * cols + col] = tile
		if new_tiles == True:
			self.drop_tiles()
	...
	# Select immediate neighbosds
	def select_bomb_from(self, tile, visited, count=1):
		tile.selected = True
		nbs = self.neighbors(tile)
		for x in nbs:
			count = count + 1
			x.selected = True
		return count
		
	def select_from(self, tile, visited, count=1,bb=True ):
